...
// SVG Theme Toggle Icon (unchanged)
...
// SVG Hamburger Menu Icon (unchanged)
const HamburgerIcon = () => {
...
const Header = ({ onToggleSidebar }) => {
    ...
    return (
        <header className="fixed w-full flex justify-between items-center px-4 sm:px-6 md:px-8 py-4 bg-white dark:bg-gray-900 shadow-md transition-colors duration-200 z-30">
            {/* Left: Hamburger Menu, Logo and Name */}
            <div className="flex items-center space-x-3">
                {/* Hamburger Menu Button - Only show when authenticated */}
                {auth?.accessToken && (
                    <button
                        className="flex items-center justify-center bg-transparent hover:bg-gray-100 dark:hover:bg-gray-700 p-2 rounded-lg transition-colors duration-200 focus:outline-none lg:block dark:text-white"
                ...
                <Logo />
                <span className="font-serif font-extrabold text-2xl sm:text-3xl md:text-4xl leading-none tracking-normal text-cyan-400 dark:text-cyan-300 transition-colors duration-200">
                    Blog EASE
                </span>
            </div>
            ...
            {/* Mobile Menu Button */}
            <button
                className="md:hidden flex items-center justify-center bg-transparent hover:bg-gray-100 dark:hover:bg-gray-700 p-2 rounded-lg transition-colors duration-200 focus:outline-none dark:text-white"
            ...
            {/* Mobile Menu */}
            {mobileMenuOpen && (
                <div className="md:hidden absolute top-full left-0 right-0 bg-white dark:bg-gray-900 shadow-lg py-4 px-6 flex flex-col items-center space-y-4 transition-all duration-200">
                    {auth?.accessToken ? (
                        <Link
                            to="/my-posts"
                            className="font-serif font-normal text-sm leading-none text-black dark:text-white hover:text-cyan-400 dark:hover:text-cyan-300 transition-colors duration-200 bg-transparent focus:outline-none"
                    ...
                    <Link
                        to="/about"
                        className="font-serif font-normal text-base leading-none text-black dark:text-white hover:text-cyan-400 dark:hover:text-cyan-300 transition-colors duration-200 bg-transparent focus:outline-none"
                    ...
                    {!auth?.accessToken && (
                        ...
                        >
                            <button
                                className="font-serif font-normal text-base leading-none text-black dark:text-white hover:text-white dark:hover:text-black bg-white dark:bg-gray-800 hover:bg-gradient-to-r hover:from-cyan-400 hover:to-purple-500 px-4 py-2 border-2 border-transparent bg-gradient-to-r from-cyan-400 to-purple-500 bg-clip-padding rounded-lg transition-all duration-200"
                                style={{
                                    background: "white",
                                    backgroundImage:
                                        "linear-gradient(white, white), linear-gradient(90deg, #36c5d1, #A82ED3)",
